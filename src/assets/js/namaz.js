...
const createNamazNiyatCard = ({ subtitle, arabic, pronunciation }) => {
    ...
    cardElement.innerHTML = `
        <h3 class="text-lg font-semibold mb-3 text-center">${subtitle}</h3>
        <p class="font-arabic text-lg md:text-xl leading-[30px] md:!leading-8 text-center mb-2 font-semibold" dir="rtl">${arabic}</p>
        <p class="font-normal text-center text-lg leading-[26px]"><strong>উচ্চারণঃ </strong>${pronunciation}</p>
    `;
    return cardElement
}
...
const createTagElemnt = ({ tagName, dataType }, isActive) => {
    ...
    li.innerHTML = `
        <button class="font-medium text-[15px] md:text-base text-left py-[6px] px-3 rounded-md block w-full filter-button ${isActive ? "active" : ""}" data-type="${dataType}">${tagName}</button>
    `
    return li
}
